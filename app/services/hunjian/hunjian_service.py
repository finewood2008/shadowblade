...
from tools.utils import random_with_system_time, extent_audio, run_ffmpeg_command
...
def get_session_video_scene_text(scene_number=0, scene_folders=None, scene_texts=None):
    """Return video directory list and text list from explicit parameters.

    Args:
        scene_number: number of scenes (0-based max index)
        scene_folders: dict mapping scene index (1-based) to folder path
        scene_texts: dict mapping scene index (1-based) to text file path
    """
    video_dir_list = []
    video_text_list = []
    scene_folders = scene_folders or {}
    scene_texts = scene_texts or {}
    for i in range(int(scene_number) + 1):
        print("select video scene " + str(i + 1))
        folder = scene_folders.get(i + 1)
        if folder is not None:
            video_dir_list.append(folder)
            video_text_list.append(scene_texts.get(i + 1))
...
def get_audio_and_video_list(audio_service, audio_rate, audio_voice=None,
                             scene_number=0, scene_folders=None, scene_texts=None):
    audio_output_file_list = []
    video_dir_list, video_text_list = get_session_video_scene_text(scene_number, scene_folders, scene_texts)
    video_scene_text_list = get_video_scene_text_list(video_text_list)
    if not audio_voice:
        raise ValueError("请先设置配音语音")
    i = 0
    for video_scene_text in video_scene_text_list:
        if video_scene_text is not None and video_scene_text != "":
            ...
            raise RuntimeError("配音文字不能为空")

    return audio_output_file_list, video_dir_list


def get_audio_and_video_list_local(audio_service, scene_number=0,
                                    scene_folders=None, scene_texts=None):
    audio_output_file_list = []
    video_dir_list, video_text_list = get_session_video_scene_text(scene_number, scene_folders, scene_texts)
    video_scene_text_list = get_video_scene_text_list(video_text_list)
    i = 0
...
def get_video_text(scene_number=0, scene_folders=None, scene_texts=None):
    video_dir_list, video_text_list = get_session_video_scene_text(scene_number, scene_folders, scene_texts)
    video_scene_text_list = get_video_scene_text_list(video_text_list)
